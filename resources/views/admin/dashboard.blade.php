@extends('layouts.app')

@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-semibold">Admin Overview</h1>
    <p class="text-gray-500">System health and live activity at a glance.</p>
</div>

<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
    <div class="bg-white rounded shadow p-4">
        <div class="text-sm text-gray-500">Live listeners</div>
        <div class="text-3xl font-semibold">{{ $listenersLive }}</div>
    </div>
    <div class="bg-white rounded shadow p-4">
        <div class="text-sm text-gray-500">Active stations</div>
        <div class="text-3xl font-semibold">{{ $stations }}</div>
    </div>
    <div class="bg-white rounded shadow p-4">
        <div class="text-sm text-gray-500">Alerts</div>
        <div class="text-3xl font-semibold">{{ $alerts }}</div>
    </div>
</div>

<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <div class="bg-white rounded shadow">
        <div class="px-4 py-3 border-b">
            <h2 class="font-semibold">Live sessions</h2>
        </div>
        <div class="p-4">
            @if($liveSessions->isEmpty())
                <p class="text-gray-500">No live sessions right now.</p>
            @else
                <table class="min-w-full text-sm">
                    <thead>
                    <tr class="border-b text-left">
                        <th class="py-2">Station</th>
                        <th class="py-2">Streamer</th>
                        <th class="py-2 text-center">Listeners</th>
                        <th class="py-2 text-right">Started</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($liveSessions as $session)
                        <tr class="border-b">
                            <td class="py-2">{{ $session->station?->name }}</td>
                            <td class="py-2">{{ $session->streamer?->name ?? 'Auto DJ' }}</td>
                            <td class="py-2 text-center">{{ $session->listeners_current }}</td>
                            <td class="py-2 text-right">{{ optional($session->started_at)->diffForHumans() }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
            @endif
        </div>
    </div>

    <div class="bg-white rounded shadow">
        <div class="px-4 py-3 border-b">
            <h2 class="font-semibold">Platform snapshot</h2>
        </div>
        <div class="p-4 grid grid-cols-2 gap-4 text-sm">
            <div>
                <div class="text-gray-500">Tenants</div>
                <div class="text-lg font-semibold">{{ $tenants }}</div>
            </div>
            <div>
                <div class="text-gray-500">Nodes</div>
                <div class="text-lg font-semibold">{{ $nodes }}</div>
            </div>
            <div>
                <div class="text-gray-500">Domains</div>
                <div class="text-lg font-semibold">{{ $domains }}</div>
            </div>
            <div>
                <div class="text-gray-500">Streamers</div>
                <div class="text-lg font-semibold">{{ $streamers }}</div>
            </div>
        </div>
        <div class="border-t px-4 py-3 text-sm">
            <a href="{{ route('admin.guide') }}" class="text-indigo-600">Go Live step-by-step guide</a>
        </div>
    </div>
</div>
@endsection
